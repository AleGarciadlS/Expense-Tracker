...
export function ConfirmDrawer({open,setOpen,file,ocr}:{open:boolean; setOpen:(v:boolean)=>void; file:File|null; ocr:any}){
  ...
  const [amountStr,setAmountStr] = useState('') // editable string
  ...
  // Load categories
  useEffect(()=>{ (async()=>{
    const cats = await db.categories.where('active').equals(true).toArray()
    setCategories(cats)
    if (!cats.find(c=>c.id===categoryId) && cats.length) setCategoryId(cats[0].id)
  })() }, [])
  ...
  return (
    <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'flex-end', justifyContent:'center'}}>
      <div style={{background:'#1f2937', width:'100%', maxWidth:'36rem', padding:'1rem', borderRadius:'1rem', border:'1px solid #334155'}}>
        ...
        {/* Fallback button shown if OCR was low/empty */}
        {lowConfidence && (
          <div style={{marginBottom:'0.5rem'}}>
            <button
              onClick={()=>{/* we are already manual-friendly; this just clears any partial values if user wants */ setMerchant(''); setDate(''); setCurrency('UYU'); setAmountStr('') }}
