...
export function ConfirmDrawer({
  open, setOpen, file, ocr,
}: { open: boolean; setOpen: (v: boolean) => void; file: File | null; ocr: any }) {
  ...
  return (
    <div
      style={{
        ...
        zIndex: 9999, // keep overlay above charts/tooltips
      }}
      role="dialog"
      aria-modal="true"
    >
      <div
        style={{
          ...
          zIndex: 10000, // ensure panel is topmost
        }}
      >
        <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.75rem' }}>
          Confirm expense{' '}
          {lowConfidence && (
            <span style={{ marginLeft: '0.5rem', color: '#fbbf24', fontSize: '0.875rem' }}>
              Low confidence — you can type manually
            </span>
          )}
        </h3>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'end', marginTop: '1rem' }}>
          <button
            style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid #475569', background: 'transparent', color: '#e2e8f0' }}
          ...
          <button
            disabled={busy}
            style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', background: '#e2e8f0', color: '#0b1222' }}
            onClick={onSave}
          >
            {busy ? 'Saving…' : 'Save'}
          </button>
